...
### Fit data with the 3eqn model using LSE and  MCMC methds and save results [H2]
...
########################################################################################################################################################
#########################################################################################################################################################
##### [H2.] Fit data with the 3eqn model using LSE and  MCMC methds and save results
#########################################################################################################################################################
...
p_vary<-list(um=0.098,umd=0.01,F=2.55,d=0.08,w=1.0,T_val=80) #To vary during fitting
...
###############################################################################################################
# LSE-Model fitting using the aestivation model: H2 

#um,umd,F,d,w,Kc upper and lower parameter bounce
#LB<-c(0.05,0.005,10.00,0.07,0.01,15.0)
#UB<-c(0.10,0.03,20.00,0.10,0.50,20.0)

LB<-c(0.05,0.005,01.00,0.01,0.50,70)
UB<-c(0.10,0.025,20.00,0.10,1.00,100)

p_start<-c(um=0.098,umd=0.01,F=2.55,d=0.08,w=1,T_val=80) #Intial parameters guesses
...
par(new = TRUE)
plot(Vector_popltn$Time,Vector_popltn$Active,type='l',ylim=c(0,8),lwd=2,col='red',xlab='Time',ylab='Adult Mosquitoes') #plot output
lines(Vector_popltn$Time,Vector_popltn$Dormant,type='l',lwd=2,col='black')
...
###############################################################################################################
# MCMC-Model fitting using the aestivation model: H2 
...
MCMC <- modMCMC(p=p_est,f=Posisson_Log_LKL_3Eqn_active,niter=110000,lower=LB,upper=UB,wvar0 = 1,var0=NULL,updatecov=50,jump=p_est*0.25,burninlength = 10000,ntrydr = 2)
...
Save_plots_results_aestivation(LBq,LBq50,UBq50,UBq,p_mcmc,file_name='H2',path_name,y_lim=10.0)
