...
Mosq_data <- read.csv("~/Documemts/Data/Sudan_data/Adult_Female_Dry-Wet.csv")
Rain_Sudan <- read.csv("~/Documemts/DataSudan_data/Mean-dairly-data-Khortum-district.csv")
...
p_vary<-list(um=0.05,umd=0.01,F=15,d=0.02) #To vary during fitting
...
plot(Vector_popltn$Time,Vector_popltn$Active,type='l',ylim=c(0,330),lwd=2,col='red',xlab='Time',ylab='Adult Mosquitoes') #plot output
...
LB<-c(0.035,0.0035,01.00,0.005)
UB<-c(0.10,0.025,25.00,0.50)

Aestivation_Model_Used<-Vector_3qn_seasonal_Eff

p_start<-c(um=0.08,umd=0.008,F=15,d=0.08) #Intial parameters guesses
...
#MCMC <- modMCMC(p=p_est,f=Gaussian_Log_LKL_3Eqn_Aestivation,niter=100000,lower=LB,upper=UB, var0=NULL,wvar0 = 1,updatecov=100,ntrydr=5,burninlength=10000,jump=p_est*0.5)

MCMC <- modMCMC(p=p_est,f=Posisson_Log_LKL_3Eqn_Aestivation,niter=100000,lower=LB,upper=UB,wvar0 = 1,var0=NULL,updatecov=30,ntrydr=2,burninlength=10000,jump=p_est*0.5)
...
#Save_plots_results_aestivation(LBq,LBq50,UBq50,UBq,p_mcmc,file_name='H2_dry_All_Fattasha',path_name,y_lim=120)
Save_plots_results_aestivation(LBq,LBq50,UBq50,UBq,p_mcmc,file_name='H2_dry4ip_All_Fattasha',path_name,y_lim=330)

#Save chain and distributions
path_name<-'~/Documemts/Results/MCMC_'
save_name<-'H2_dry4ip_All_Fattasha'
